...
router.get("/comedy", async (req, res) => {
  const newsfeed = await Newsfeed.find({ category: "comedy" }).sort({
    date: -1
  });
  try {
    if (newsfeed == null || newsfeed.length == 0) {
...
router.get("/lifestyle", async (req, res) => {
  const newsfeed = await Newsfeed.find({ category: "lifestyle" }).sort({
    date: -1
  });
  try {
    if (newsfeed == null || newsfeed.length == 0) {
...
router.get("/sport", async (req, res) => {
  const newsfeed = await Newsfeed.find({ category: "sport" }).sort({
    date: -1
  });
  try {
    if (newsfeed == null || newsfeed.length == 0) {
...
router.get("/business", async (req, res) => {
  const newsfeed = await Newsfeed.find({ category: "business" }).sort({
    date: -1
  });
  try {
    if (newsfeed == null || newsfeed.length == 0) {
